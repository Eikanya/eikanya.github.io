...
(function (LIVE2DCUBISMPIXI) {
  ...
  var ModelBuilder = (function () {
    ...
    ModelBuilder.prototype.buildFromModel3Json = function (loader, model3Obj, callbackFunc) {
      ...
      var textureCount = 0
      if (typeof (model3Obj.data.FileReferences.Moc) !== 'undefined') { loader.add('moc', modelDir + model3Obj.data.FileReferences.Moc, { xhrType: PIXI.loaderResource.XHR_RESPONSE_TYPE.BUFFER }) }
          ...
          for (const group in model3Obj.FileReferences.Motions) {
            model3Obj.FileReferences.Motions[group].forEach((element) => {
              ...
              if (!motionNames.includes(name + '_' + motionName)) {
                ...
                loader.add(n, modelDir + element.File, { xhrType: PIXI.LoaderResource.XHR_RESPONSE_TYPE.JSON })
                motionNames.push(name + '_' + motionName)
              }
            })
          }
...
class l2dViewer {
  constructor ({ basePath, modelName, width, height, el }) {
    ...
    window.onresize = (event) => {
      ...
      if (this.model) {
        this.model.position = new PIXI.Point((width * 0.6), (height * 0.5))
        this.model.scale = new PIXI.Point((this.model.position.x * 0.13), (this.model.position.x * 0.12))
    ...
    this.app.view.addEventListener('mouseup', (event) => {
      ...
      if (this.isClick) {
        ...
        } else if (this.isHit('Touch_Special', event.offsetX, event.offsetY)) {
